            ...
            try:
                ...
                if compile_res.returncode == 0:
                    ...
                    wants_pr = self.notification_service.ask_confirmation(
                        "Dataform Scout",
                        f"Fix successful on attempt {attempt}. "
                        "Would you like to open a Pull Request?",
                    )
                    if wants_pr:
                        title = (
                            f"Fix error in "
                            f"{action_name or sqlx_path or 'Dataform pipeline'}"
                        )
                        body = (
                            "Automated fix generated by Dataform Scout.\n\n"
                            f"Error resolved:\n```\n{error_msg}\n```"
                        )
                        try:
                            self.git_ops.push_and_create_pr(
                                branch, wt_path, title, body
                            )
                            self.notification_service.notify(
                                "Dataform Scout",
                                "Pull Request Created",
                                f"Branch {branch} has been pushed.",
                            )
