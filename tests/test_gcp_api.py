...
def test_get_gcp_repo_url_success(
    mock_urlopen: MagicMock, mock_token: MagicMock
) -> None:
    mock_response = MagicMock()
    mock_response.read.return_value = json.dumps(
        {
            "gitRemoteSettings": {
                "url": "https://source.developers.google.com/p/my-proj/r/my-repo"
            }
        }
    ).encode("utf-8")
    ...
    client = GCPApiClient()
    url = client.get_gcp_repo_url("my-proj", "eu", "my-repo")
...
def test_fetch_workflow_branch(mock_urlopen: MagicMock, mock_token: MagicMock) -> None:
    ...
    client = GCPApiClient()
    branch = client.fetch_workflow_branch("my-proj", "eu", "my-repo", "inv-123")
    assert branch == "my-feature-branch"


@patch("src.gcp_api.urllib.request.urlopen")
def test_fetch_workflow_failed_actions(
    mock_urlopen: MagicMock, mock_token: MagicMock
) -> None:
    mock_response = MagicMock()
    mock_response.read.return_value = json.dumps(
    ...
    client = GCPApiClient()
    failed = client.fetch_workflow_failed_actions("my-proj", "eu", "my-repo", "inv-123")
    assert len(failed) == 1
